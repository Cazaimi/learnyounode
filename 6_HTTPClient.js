...
function httpGet(url) {
	http.get(url, function(response){
		...
		/*Setting the data encoding because a buffer is received in 
		response to the HTTP request. This will convert it into a string.
		This could have also been achieved using data.toString();*/
		response.setEncoding('utf8');
		
		/*The HTTP request emits events. It sends data in 'chunks', like light 
		sends photons. Hence, each time a chunk is available, the code inside
		response.on('data', ...) function will run.*/
		response.on("data", function(data){
			console.log(data);
		})
		response.on("error", function(e){
			console.log('Following error was found:' + e.message);
		});
