...
function httpGet(url) {
	http.get(url, function(response){
		/*This will store the totality of the data sent by the server.*/
		let totalData = '';
		
		response.setEncoding('utf8');
		
		/*HTTP works by sending the data in 'chunks'. If we call the 
		response.on('data', ... {console.log(data);}) once, only the first chunk captured 
		by the response will be printed. Since we want the entire data here, we 
		have to append the chunk we have received, each time it is available, which is
		what we do here.*/
		response.on("data", function(data) {
			totalData += data; 
		})
		
		/*The chunks will keep getting appended until there is nothing else left
		to transfer. However, how will we detect this? This is done using 
		repsonse.on('end', ...). This will be called after all the data has been sent.*/
